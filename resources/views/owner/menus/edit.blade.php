...
<div class="max-w-2xl mx-auto p-6 ml-64">
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('owner.menus.update', $menu->id) }}" class="bg-white shadow rounded p-6 space-y-4">
        ...
        <input type="text" name="image_url" placeholder="https://example.com/image.jpg" value="{{ old('image_url', $menu->image) }}" class="w-full border border-gray-300 p-2 rounded" />

        <div class="flex items-center space-x-4 mt-4">
            @if($menu->image)
                <img src="{{ asset($menu->image) }}" alt="Current Image" class="w-20 h-20 object-cover border rounded" />
            @else
