                    ...
                    <div class="flex items-center gap-4 border-b pb-4">
                        ...
                        <div class="flex flex-col items-end gap-2">
                            <div class="flex items-center gap-1">
                                <button wire:click="decrement({{ $item->id }})" class="bg-gray-200 hover:bg-gray-300 rounded px-2 py-1">-</button>
                                <span class="w-6 text-center">{{ $item->quantity }}</span>
                                <button wire:click="increment({{ $item->id }})" class="bg-gray-200 hover:bg-gray-300 rounded px-2 py-1">+</button>
                            </div>
                            <button wire:click="remove({{ $item->id }})" class="text-red-500 hover:text-red-700 text-sm mt-1">Remove</button>
                            <span class="font-bold text-orange-600">Rs {{ number_format($item->menu->price * $item->quantity, 2) }}</span>
                        </div>
                    </div>
                @empty
        ...
        <aside class="w-full lg:w-96 bg-white rounded-lg shadow p-6 flex flex-col gap-6 sticky top-6 self-start">
            ...
            <div class="space-y-2">
                <div class="flex justify-between">
                    <span>Subtotal</span>
                    <span>Rs {{ number_format($this->cartItems->sum(fn($i) => $i->menu->price * $i->quantity), 2) }}</span>
                </div>
                <div class="flex justify-between">
                    <span>Taxes (10%)</span>
                    <span>Rs {{ number_format($this->cartItems->sum(fn($i) => $i->menu->price * $i->quantity) * 0.1, 2) }}</span>
                </div>
                <div class="flex justify-between font-bold text-lg border-t pt-2">
                    <span>Total</span>
                    <span>Rs {{ number_format($this->cartItems->sum(fn($i) => $i->menu->price * $i->quantity) * 1.1, 2) }}</span>
                </div>
            </div>
